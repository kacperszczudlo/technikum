window.onload = function(){
    document.querySelector("#start").onclick = function(){
        ...
        if(!isNaN(rows) && !isNaN(cols) && rows>=3 && rows<=100 && cols>=3 && cols<=100){
            document.querySelector("#tab").innerHTML = generTab(rows,cols);
            const tds = document.querySelectorAll("#tab td");
            //console.log(tds);
            for(let i=0;i<tds.length;i++){
                tds[i].onmousedown = function(e){
                    //console.log(e);
                    if (this.innerHTML!="" && e.buttons!=3) return;
                    switch(e.buttons){
                        case 1 : this.innerHTML = "<img src='x.png' alt='x'>";break;
                        case 2 : this.innerHTML = "<img src='o.png' alt='o'>";break;
